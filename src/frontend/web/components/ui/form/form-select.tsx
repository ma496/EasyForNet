    ...
    <div className={cn(className, (isDirty || submitCount > 0) && hasError && 'has-error')} ref={containerRef}>
      ...
      <div className={cn('relative text-white-dark', 'custom-select')}>
        <button
          type="button"
          className={cn(
            'form-input flex min-h-10 w-full cursor-pointer items-center gap-1 bg-transparent pr-10 ltr:text-left rtl:text-right',
            icon && 'ps-10',
        ...
        >
          ...
          <span className={cn('flex-1 truncate ltr:text-left rtl:text-right', !field.value && 'text-gray-400')}>{field.value ? getSelectedLabel() : placeholder}</span>
